				...
				<div class="panel panel-bordered">
					<form method="post" id="userForm">
						<div class="panel-heading">
							<h3 class="panel-title">회원관리 알림톡 설정</h3>
							<div class="panel-actions panel-actions-keep">
								<button type="button" class="btn btn-primary" name="save_notification_btn" data-form="userForm" data-title="회원관리 알림톡">저장</button>
							...
							<table class="table table-striped">
								...
								<tbody>
									<c:forEach var="userNoti" items="${userNotiList}" varStatus="status">
										<tr>
											<td class="text-center align-middle">${userNoti.action_name}</td>
											<td class="text-center align-middle">
												<input type="text" class="form-control" placeholder="템플릿 ID" autocomplete="off" name="data[${status.index}][template_id]" value="${userNoti.template_id}" />
											</td>
											<td class="text-center align-middle">
												<span class="inline-block">
													<input type="checkbox" data-color="#17b3a3" data-plugin="switchery" name="data[${status.index}][on_off]" value="Y" <c:if test="${userNoti.on_off eq 'Y'}">checked</c:if> />
												</span>
												<input type="hidden" name="data[${status.index}][idx]" value="${userNoti.idx}" />
											</td>
										</tr>
									</c:forEach>
								</tbody>
...
<script type="text/javascript">
	...
	for (var i = 0; i < save_btn_all.length; i++) {
		...
		save_btn.addEventListener("click", function() {
			if (!submitConfirm($(save_btn)))
				return false;

			var form_id = save_btn.dataset.form;
			var form_data = $("#" + form_id).serializeObject();

			var request = $.ajax({ url : "/notification/modifyNotification.do", method : "post", contentType : "application/json",
